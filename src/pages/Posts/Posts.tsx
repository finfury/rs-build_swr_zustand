...
const Posts = () => {
  ...
  const response = useSWR<AxiosResponse<Post[]>>(`/posts?_limit=5&_page=${pageIndex}`, fetchGetPosts);
    ...
    <div className={styles.posts}>
      <div className={styles.section}>
        ...
        <div className={styles.buttons}>
          <Button text='Предыдущая страница' variant='outlined' onClick={prevPageClickHandler} />
          <Button
            text='Обновить посты'
            variant='outlined'
            type='button'
          ...
          />
          <Button text='Следующая страница' variant='outlined' onClick={nextPageClickHandler} />
        </div>
      </div>
    </div>
  );
